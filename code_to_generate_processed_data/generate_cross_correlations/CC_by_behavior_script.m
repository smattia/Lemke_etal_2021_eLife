...
for day = 1:8
    ...
    %%% LOAD SPIKING DATA
        ...
        m1_spiking_width = cell(16,max(plx_spiking(:,2)));
        count = 1;
        for m1_chan = 1:16
            for m1_unit = 1:max(plx_spiking(:,2))
                m1_spiking_width{count,m1_unit} = tmp_width{m1_chan,pre_sleep_blocks{day}}(tmp_sort{m1_chan,pre_sleep_blocks{day}}==m1_unit);
            end
            count = count + 1;
        end
        
        dls_spiking_width = cell(16,max(plx_spiking(:,2)));
        count = 1;
        for dls_chan = 17:32
            for dls_unit = 1:max(plx_spiking(:,2))
                dls_spiking_width{count,dls_unit} = tmp_width{dls_chan,pre_sleep_blocks{day}}(tmp_sort{dls_chan,pre_sleep_blocks{day}}==dls_unit);
            end
            count = count + 1;
        end
        
        m1_spiking_peak2valley = cell(16,max(plx_spiking(:,2)));
        count = 1;
        for m1_chan = 1:16
            for m1_unit = 1:max(plx_spiking(:,2))
                m1_spiking_peak2valley{count,m1_unit} = tmp_peak2valley{m1_chan,pre_sleep_blocks{day}}(tmp_sort{m1_chan,pre_sleep_blocks{day}}==m1_unit);
            end
            count = count + 1;
        end
        
        dls_spiking_peak2valley = cell(16,max(plx_spiking(:,2)));
        count = 1;
        for dls_chan = 17:32
            for dls_unit = 1:max(plx_spiking(:,2))
                dls_spiking_peak2valley{count,dls_unit} = tmp_peak2valley{dls_chan,pre_sleep_blocks{day}}(tmp_sort{dls_chan,pre_sleep_blocks{day}}==dls_unit);
            end
            count = count + 1;
        end
        
        [all_pairs] = CC_by_behavior(pre_m1_spiking, pre_dls_spiking, post_m1_spiking, post_dls_spiking, pre_beh_state, post_beh_state, pre_video_start_stop, post_video_start_stop,m1_spiking_width,dls_spiking_width,m1_spiking_peak2valley,dls_spiking_peak2valley);
   ...
   %%% SAVE
       ...
       save('CC_by_behavior.mat','all_pairs','-v7.3');
...
for day = 3:17
    ...
    %%% LOAD SPIKING DATA
        ...
        tmp_width = cell(64,length(tmp_day_blocks));
        tmp_peak2valley = cell(64,length(tmp_day_blocks));
        for chan = 1:64
            chan_sorts = plx_spiking(plx_spiking(:,1)==chan,:);
            for block = 1:length(tmp_day_blocks)
                tmp_sort{chan,block} = chan_sorts(1+sum(tmp_length(chan,1:block)):sum(tmp_length(chan,1:block+1)),2);
...
for day = [4:13]
    ...
    %%% LOAD SPIKING DATA
        ...
        m1_spiking_width = cell(32,max(plx_spiking(:,2)));
        count = 1;
        for m1_chan = [8 4 12 5 22 6 21 17 13 14 28 1 30 25 29 20 52 53 54 61 49 41 38 36 62 57 60 44 37 46 45 33]
            for m1_unit = 1:max(plx_spiking(:,2))
                m1_spiking_width{count,m1_unit} = tmp_width{m1_chan,pre_sleep_plx_blocks{day}}(tmp_sort{m1_chan,pre_sleep_plx_blocks{day}}==m1_unit);
            end
            count = count + 1;
        end
        
        dls_spiking_width = cell(32,max(plx_spiking(:,2)));
        count = 1;
        for dls_chan = [2 3 9 7 11 10 16 15 19 18 24 23 27 26 32 31 35 34 40 39 43 42 48 47 51 50 56 55 59 58 64 63]
            for dls_unit = 1:max(plx_spiking(:,2))
                dls_spiking_width{count,dls_unit} = tmp_width{dls_chan,pre_sleep_plx_blocks{day}}(tmp_sort{dls_chan,pre_sleep_plx_blocks{day}}==dls_unit);
            end
            count = count + 1;
        end
        
        m1_spiking_peak2valley = cell(32,max(plx_spiking(:,2)));
        count = 1;
        for m1_chan = [8 4 12 5 22 6 21 17 13 14 28 1 30 25 29 20 52 53 54 61 49 41 38 36 62 57 60 44 37 46 45 33]
            for m1_unit = 1:max(plx_spiking(:,2))
                m1_spiking_peak2valley{count,m1_unit} = tmp_peak2valley{m1_chan,pre_sleep_plx_blocks{day}}(tmp_sort{m1_chan,pre_sleep_plx_blocks{day}}==m1_unit);
            end
            count = count + 1;
        end
        
        dls_spiking_peak2valley = cell(32,max(plx_spiking(:,2)));
        count = 1;
        for dls_chan = [2 3 9 7 11 10 16 15 19 18 24 23 27 26 32 31 35 34 40 39 43 42 48 47 51 50 56 55 59 58 64 63]
            for dls_unit = 1:max(plx_spiking(:,2))
                dls_spiking_peak2valley{count,dls_unit} = tmp_peak2valley{dls_chan,pre_sleep_plx_blocks{day}}(tmp_sort{dls_chan,pre_sleep_plx_blocks{day}}==dls_unit);
            end
            count = count + 1;
        end
...
for day = [1:9]
    ...
    %%% LOAD SPIKING DATA
        ...
        [all_pairs] = CC_by_behavior_kilosort(pre_m1_spiking, pre_dls_spiking, post_m1_spiking, post_dls_spiking, pre_beh_state, post_beh_state, pre_video_start_stop, post_video_start_stop);
